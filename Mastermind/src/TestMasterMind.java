   ...
   //   testComputerGuess(); 
    ...
    private static void testPromptInt() {
        boolean error = false;
         ...
         //check whether the integer between min and max is returned.
            ...
            int expected1 = 8;
            int result1 = MasterMind.promptInt(in, "Enter integer: ", 5, 15);
            if ( expected1 != result1) {
                System.out.println("1) testPromptInt expected: " + expected1 + " result: " + result1);
                error = true;
            }
            
        Scanner inn = new Scanner("104 304 -234 10");
        
            int expected2 = 10;
        int result2 = MasterMind.promptInt(in, "Enter integer: ", 5, 15);
    ...
    private static void testIndexOf() {
        ...
        { //check whether the index of 'b' is returned.
            char [] list = {'a', 'A', 'b','B'};
            int expected1 = 2;
            int result1 = MasterMind.indexOf( list, 'b');
            if ( expected1 != result1) {
                System.out.println("1) testIndexOf expected: " + expected1 + " result: " + result1);
                error = true;
            }
        
            char [] list2 = {'E', 'e', 'e','r'};
            int expected2 = 1;
            int result2 = MasterMind.indexOf( list2, 'e');
            if ( expected2 != result2) {
                System.out.println("1) testIndexOf expected: " + expected2 + " result: " + result2);
                error = true;
            }
    ...
    private static void testGenerateHiddenCode() {
        ...
        {   //"randomly" chooses 3 symbols from the list
            ...
            char [] expected = {'C', 'A', 'D'};
            char [] result = MasterMind.generateHiddenCode( rand1, numPositions, symbols);
            if ( !Arrays.equals( expected,  result)) {
                System.out.println("1) testGenerateHiddenCode expected: " + Arrays.toString(expected) + " result: " + Arrays.toString(result));
                error = true;
            }
            Random rand2 = new Random(2345);
            int numPositions2 = 5;
            char [] symbols2 = {'A', 'B', 'C', 'D','E'};
            char [] expected2 = {'A', 'E', 'D','E','C'};
            char [] result2 = MasterMind.generateHiddenCode( rand2, numPositions2, symbols2);
            if ( !Arrays.equals( expected2,  result2)) {
                System.out.println("1) testGenerateHiddenCode expected: " + Arrays.toString(expected2) + " result: " + Arrays.toString(result2));
                error = true;
            }
    ...
    private static void testIsValidCode() {
        ...
        { //check whether input of 123 is valid
            char[] input1 = {'1','a','a','a'};
            char[] symbols1 = {'0','1','2','3','4'};
            boolean expected1 = false;
            boolean result1 = MasterMind.isValidCode(3, symbols1, input1);
            if ( result1 != expected1) {
                System.out.println("1) testIsValidCode expected: " + expected1 + " result: " + result1);
                error = true;
                
            }
            
            char[] input2 = {'1','f','5','3'};
            char[] symbols2 = {'1','f','5','3','4'};
            boolean expected2 = true;
            boolean result2 = MasterMind.isValidCode(4, symbols2, input2);
            if ( result2 != expected2) {
                System.out.println("1) testIsValidCode expected: " + expected2 + " result: " + result2);
                error = true;
    ...
    private static void testPromptForGuess() {
        ...
        {  //check whether input of 123 is correctly read in
            Scanner in1 = new Scanner("123\n");
            char[] expected1 = {'1','2','3'};
            char[] symbols1 = {'0','1','2','3','4'};
            char[] result1 = MasterMind.promptForGuess(in1, "Enter guess:", 3, symbols1);
            if ( !Arrays.equals( expected1, result1)) {
                System.out.println("1) testPromptForGuess expected: " + Arrays.toString(expected1) + " result: " + Arrays.toString(result1));
                error = true;
            }
            Scanner in2 = new Scanner("    567\n");
            char[] expected2 = {'5','6','7'};
            char[] symbols2 = {'2','5','5','6','7'};
            char[] result2 = MasterMind.promptForGuess(in2, "Enter guess:", 3, symbols2);
            if ( !Arrays.equals( expected2, result2)) {
                System.out.println("1) testPromptForGuess expected: " + Arrays.toString(expected2) + " result: " + Arrays.toString(result2));
                error = true;
            }
    ...
    private static void testCountAllHits() {
        ...
        { //count the total number of hits
            ...
            char[] hiddenCode2 = {'2','5','3','6'};
            char[] guess2 = {'1','7','4','1'};
            char[] symbols2 = {'0','1','2','3','4','5','6'};
            int expected2 = 0;
            int result2 = MasterMind.countAllHits(hiddenCode2, guess2, symbols2);
            if ( expected2 != result2) {
                System.out.println("1) testCountAllHits expected: " + expected2 + " result: " + result2);
                error = true;
            }
    ...
    private static void testNextCode() {
        ...
        {  //tests with 6 numbers
            char[] symbols = {'1', '2', '3', '4', '5', '6'};
            char[] input = {'1', '4', '5', '2'};
            char[] expected = {'1', '4', '5', '3'};
            char[] result = MasterMind.nextCode(input, symbols);
            if (!Arrays.equals(expected, result)) {
                System.out.println("testNextCode 6 expected=" + Arrays.toString(expected)
                    + " result=" + Arrays.toString(result));
